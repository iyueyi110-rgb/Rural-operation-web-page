...
port_in_use() {
  if command -v lsof >/dev/null 2>&1; then
    ...
    netstat -an | grep -E "[:.]${1}[[:space:]].*LISTEN" >/dev/null 2>&1
  else
    return 1
  fi
}
...
if [ "$SKIP_INSTALL" != "1" ]; then
  if [ ! -d "node_modules" ] || [ ! -d "apps/web/node_modules" ] || [ ! -d "apps/admin/node_modules" ]; then
    info "首次运行，安装所有依赖..."
    pnpm install
  else
