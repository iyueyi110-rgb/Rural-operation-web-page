...
async function markdownFiles(directory) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory() && !shouldScanDocumentationDirectory(entry.name))
      continue
...
function localTargets(markdown) {
  ...
  for (const [index, line] of markdown.split(/\r?\n/u).entries()) {
    ...
    if (fenceMatch) {
      fence =
        fence === null ? fenceMatch[1] : fence === fenceMatch[1] ? null : fence
      continue
    }
    if (fence) continue
    const linkPattern =
...
for (const file of files) {
  ...
  failures.push(...inspectDocumentationText({ file: relativeFile, markdown }))
