...
export function MeDashboardClient() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchWithAuth("/api/v1/me/orders").then((response) => response.ok ? response.json() : { data: [] }),
      fetchWithAuth("/api/v1/me/adoptions").then((response) => response.ok ? response.json() : { data: [] }),
    ])
      .then(([ordersPayload, adoptionsPayload]) => {
        setOrders(Array.isArray(ordersPayload.data) ? ordersPayload.data : [])
        setAdoptions(Array.isArray(adoptionsPayload.data) ? adoptionsPayload.data : [])
      })
      .catch(() => {
        setOrders([])
        setAdoptions([])
  ...
  if (loading) {
    return <p className="mt-8 text-sm font-semibold text-ink/58">正在读取个人订单与认养记录。</p>
  }
    ...
    <div className="mt-8 grid gap-5">
      {empty ? (
        <div className="rounded-lg border border-line bg-white p-5">
          <h2 className="text-xl font-extrabold">您还没有订单，去认养一棵荔枝树吧</h2>
          <p className="mt-2 text-sm leading-6 text-ink/62">
            认养成功后，树档案、养护记录和活动提醒会汇总到这里。
          </p>
          <Link className="btn-primary mt-4 h-11 px-5" href={`/${locale}/trees`}>
            查看荔枝树认养
          </Link>
        </div>
      ) : null}
      ...
      {orders.map((order) => (
        <article className="choice-card grid gap-3 md:grid-cols-[1fr_auto]" key={order.id}>
          ...
          <div className="grid gap-2 text-sm md:text-right">
            ...
            <span className="text-ink/58">{new Date(order.createdAt).toLocaleString()}</span>
          </div>
        </article>
      ))}

      {adoptions.map((adoption) => (
        <article className="choice-card grid gap-3 md:grid-cols-[1fr_auto]" key={adoption.id}>
          <div>
            <div className="text-sm font-bold text-water">tree_adoption</div>
            <h2 className="mt-2 text-xl font-extrabold">{adoption.tree.treeCode}</h2>
            <p className="mt-2 text-sm text-ink/58">{adoption.tree.blurredLocation}</p>
          </div>
          <div className="grid gap-2 text-sm md:text-right">
            <span className="font-bold text-moss">{adoption.status}</span>
            <span className="text-ink/58">{adoption.plan}</span>
            <span className="text-ink/58">{new Date(adoption.createdAt).toLocaleString()}</span>
          </div>
        </article>
      ))}
    </div>
