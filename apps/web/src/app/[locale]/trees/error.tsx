...
export default function TreesError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-rice px-6 text-ink">
      <div className="max-w-md rounded-lg border border-stone bg-white p-6 text-center shadow-soft">
        ...
        <button className="mt-5 rounded-full bg-ink px-5 py-2 text-sm font-semibold text-white" onClick={reset} type="button">
          {t("retry")}
        </button>
      </div>
    </main>
