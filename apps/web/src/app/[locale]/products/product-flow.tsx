...
export function ProductFlow() {
  ...
  const totalAmount =
    selectedProduct?.price == null ? 0 : selectedProduct.price * quantity
  ...
  return (
    <MasterDetailLayout
      master={
        <>
          <PanelTitle icon={<Store aria-hidden="true" className="h-4 w-4" />}>
            {t("list.eyebrow")}
          </PanelTitle>
          <h2 className="mt-3 break-words text-3xl font-extrabold">
            {t("list.title")}
          </h2>
          <p className="mt-3 break-words text-sm leading-7 text-ink/68">
            {t("list.body")}
          </p>
          ...
          <div className="mt-6 grid gap-4">
            {isLoading ? (
              <SurfacePanel>{t("messages.loading")}</SurfacePanel>
            ) : null}
            {!isLoading && products.length === 0 ? (
              <EmptyState title={t("messages.loadFailed")} />
            ) : null}
            {products.map((product) => {
              ...
              return (
                <article
                  className={
                    active ? "choice-card choice-card-active" : "choice-card"
                  }
                  key={product.id}
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <h3 className="mt-3 break-words text-2xl font-extrabold">
                        {product.name}
                      </h3>
                    </div>
                    <div className="rounded-md bg-rice px-3 py-2 text-right">
                      <div className="text-lg font-extrabold text-lychee">
                        {formatPrice(product, t("list.negotiable"))}
                      </div>
                  ...
                  <button
                    className={
                      active
                        ? "btn-primary mt-5 h-10 px-5"
                        : "btn-secondary mt-5 h-10 px-5"
                    }
                    onClick={() => {
      ...
      detail={
        <>
          ...
          <div className="mt-5 rounded-md bg-rice p-4">
            ...
            <dl className="mt-4 grid gap-3 text-sm">
              ...
              <SummaryLine
                ...
                value={t("form.quantityValue", { count: quantity })}
              />
              <SummaryLine
          ...
          <div className="mt-5 rounded-md border border-stone p-4">
            ...
            <p className="mt-2 break-words text-sm leading-6 text-ink/66">
              {t("notice.body")}
            </p>
          </div>
          ...
          <button
            className="btn-primary mt-5 w-full bg-ink hover:bg-moss"
