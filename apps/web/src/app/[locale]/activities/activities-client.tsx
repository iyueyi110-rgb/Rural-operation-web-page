...
export function ActivitiesClient() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/activities?${params}`)
      .then(async (response) => {
        if (!response.ok) return { data: [], meta: { degraded: true } }
        return (await response.json().catch(() => ({ data: [] }))) as {
          data?: Activity[]
        }
      })
      .then(
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
      <div className="grid gap-4">
        ...
        {activities.map((activity) => {
          const disabled =
            activity.status === "full" || activity.status === "cancelled"
          return (
            <article className="choice-card" key={activity.id}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="status-chip border-line bg-rice text-ink/62">
              ...
              <button
                ...
                data-primary={
                  !disabled && guestName && guestPhone ? "true" : "false"
                }
      ...
      <aside className="h-fit rounded-xl border border-line bg-surface p-5 shadow-[0_12px_28px_rgba(25,32,27,0.08)] lg:sticky lg:top-20">
        ...
        {booking ? (
          <SurfacePanel className="mt-4" tone="success">
            ...
            <div className="mt-2 break-words text-sm text-ink/64">
              {booking.id}
            </div>
          </SurfacePanel>
        ) : null}
