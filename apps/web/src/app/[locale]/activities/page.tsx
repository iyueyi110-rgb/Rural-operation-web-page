...
import { HeroMeta, PanelTitle, SubpageHero, SurfacePanel } from "@web/components/subpage-ui"
import { getSiteUrl } from "@web/lib/site-url"
import { PageHeader, Section } from "@ui/index"

export async function generateMetadata({ params }: { params: { locale: Locale } }): Promise<Metadata> {
  const t = await getTranslations({ locale: params.locale, namespace: "metadata.activities" })
          ...
          <SurfacePanel className="overflow-hidden p-0">
            <div className="relative min-h-72 bg-ink text-white">
              ...
              <div className="absolute right-4 top-4 max-w-[13rem] rounded-lg border border-white/18 bg-ink/62 px-4 py-3 text-right shadow-[0_12px_28px_rgba(0,0,0,0.2)] backdrop-blur">
                <div className="text-xs font-bold text-white/56">{t("showcase.label")}</div>
                <div className="mt-1 text-lg font-extrabold leading-6">{t("showcase.name")}</div>
              </div>
              <div className="absolute bottom-5 left-5 right-5">
                <PanelTitle icon={<CalendarDays aria-hidden="true" className="h-4 w-4" />} tone="white">
                  {t("showcase.date")}
                </PanelTitle>
                <p className="mt-3 max-w-md text-sm leading-6 text-white/78">{t("showcase.body")}</p>
              </div>
            </div>
          </SurfacePanel>
        }
