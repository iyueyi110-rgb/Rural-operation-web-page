...
import {
  requireTouristRecipient,
  requireVillagerRecipient,
...
export async function GET(request: Request) {
  ...
  const validRecipientType = isRecipientType(recipientType)
    ? recipientType
    : undefined
...
export async function POST(request: Request) {
  ...
  const recipientType = isRecipientType(body.recipientType)
    ? body.recipientType
    : null
  ...
  const data = await prisma.notification.create({
    data: {
      ...
      refType:
        typeof body.refType === "string" ? body.refType.trim() || null : null,
      ...
      const auth =
        recipientType === "tourist"
          ...
          : recipientType === "villager"
            ? await requireVillagerRecipient(request, rawRecipientId)
            : {
    ...
    const auth =
      existing.recipientType === "tourist"
        ...
        : existing.recipientType === "villager"
          ? await requireVillagerRecipient(request, existing.recipientId)
          : {
              authorized: false as const,
              response: jsonResponse(
                request,
...
function isRecipientType(value: unknown): value is RecipientType {
  return (
    typeof value === "string" && recipientTypes.includes(value as RecipientType)
  )
}
...
function isNotificationCategory(value: unknown): value is NotificationCategory {
  return (
    typeof value === "string" &&
    categories.includes(value as NotificationCategory)
  )
}
...
async function resolveRecipientIds(
  ...
  return Array.from(
    new Set(
      [normalized, maskedMobile].filter((value): value is string =>
        Boolean(value),
      ),
    ),
  )
...
async function resolveSmsPhone(
  recipientType: RecipientType,
  recipientId: string,
) {
  ...
  return villager?.phone && /^1[3-9]\d{9}$/.test(villager.phone)
    ? villager.phone
    : null
}
