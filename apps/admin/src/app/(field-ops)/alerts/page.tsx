...
export default function AlertsPage() {
  ...
  const loadRows = useCallback(async () => {
    ...
    const [behaviorResult, sensorResult, weatherResult] =
      await Promise.allSettled([
        fetchWithTimeout(`${adminApiBase}/alerts?${params}`).then((response) =>
          response.json(),
        ) as Promise<{ data?: AlertRow[] }>,
        fetchWithTimeout(`${adminApiBase}/infrastructure/alerts`).then(
        ...
        fetchWithTimeout(`${adminApiBase}/weather/alerts`).then((response) =>
          response.json(),
        ) as Promise<{
          data?: Array<{
            id: string
    ...
    const behaviorRows =
      behaviorResult.status === "fulfilled"
        ? (behaviorResult.value.data ?? []).map((row) => ({
            ...row,
    ...
    const weatherRows =
      weatherResult.status === "fulfilled"
        ...
        : []
    setRows(
      [...behaviorRows, ...sensorRows, ...weatherRows].filter(
        (row) => !type || row.alertType === type,
      ),
    )
    setIsLoading(false)
  }, [status, type])
  ...
  async function updateStatus(id: string, nextStatus: string) {
    try {
      ...
      setMessage("告警状态没有更新成功，请检查网络后重新提交。")
    }
  }
  ...
  const columns: Array<TableColumn<AlertRow>> = [
    ...
    {
      key: "alertType",
      label: adminCopy.alerts.type,
      render: (value) =>
        adminCopy.alerts.types[value as keyof typeof adminCopy.alerts.types] ??
        String(value),
    },
    ...
    },
    {
      ...
      render: (_value, row) => (
        <span className="flex gap-2">
          {row.source === "behavior" && row.status === "active" ? (
            <button
          ...
          {row.source === "behavior" && row.status === "acknowledged" ? (
            <button
              className="text-moss"
              {...adminWriteControlProps(canWrite)}
  ...
  return (
    <AdminPageShell
      description="查看行为、传感器和天气告警；现场确认后记录处理结果。"
    ...
    >
      ...
      <AdminPanel className="grid gap-3 p-4 md:grid-cols-2">
        ...
        >
          ...
          {Object.entries(adminCopy.alerts.types).map(([value, label]) => (
      ...
      <AdminDataTable
        columns={columns}
        emptyLabel={adminCopy.alerts.noData}
        isLoading={isLoading}
        rows={rows}
      />
    </AdminPageShell>
