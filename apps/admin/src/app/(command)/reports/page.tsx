...
export default function ReportsPage() {
  ...
  async function loadReports() {
    ...
    try {
      ...
      setIsDemo(result.meta?.degraded === true || result.meta?.demo === true)
    } catch (caughtError) {
      setError(
  ...
  return (
    ...
    >
      ...
      {isDemo ? (
        <AdminNotice>当前暂无正式日报，已展示静态降级演示模板。</AdminNotice>
      ) : null}
          ...
          <div className="grid gap-2">
            {reports.map((report) => (
              <button
                className={
                  selected?.id === report.id
                    ? "rounded-md bg-rice p-3 text-left"
                    : "rounded-md p-3 text-left hover:bg-rice"
                }
                key={report.id}
              ...
              >
                ...
                <div className="mt-1 truncate text-xs font-semibold text-ink/54">
                  {report.title}
                </div>
              </button>
            ))}
          ...
          {selected ? (
            <div>
              ...
              <h3 className="mt-6 text-lg font-extrabold">
                {adminCopy.reports.actions}
              </h3>
              <div className="mt-3 grid gap-2">
                {selected.actionItems.map((item) => (
                  <div
                    className="rounded-md bg-rice p-3 text-sm font-semibold text-ink/70"
                    key={item.action}
                  >
                    ...
                    {item.deadline ? (
                      <span className="text-ink/45"> / {item.deadline}</span>
                    ) : null}
                  </div>
                ))}
              </div>
              <h3 className="mt-6 text-lg font-extrabold">
                {adminCopy.reports.sections}
              </h3>
              <div className="mt-3 grid gap-3">
                {selected.sections.map((section) => (
                  <section
                    className="rounded-md border border-stone p-3"
