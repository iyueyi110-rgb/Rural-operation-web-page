...
import { AlertTriangle, CheckCircle2, ClipboardList, LayoutDashboard, RefreshCw, SendHorizontal, Settings, Ticket, Trees, Users } from "lucide-react"
import { useEffect, useMemo, useState, type ReactNode } from "react"

import { adminCopy } from "@admin/lib/admin-copy"
import type { FeedbackRecord } from "@admin/lib/feedback-types"
import type { Feedback } from "@zouma/contracts"

type FeedbackStatus = Feedback["status"]

const apiBase = process.env.NEXT_PUBLIC_WEB_API_BASE ?? "http://localhost:3004/api/v1"
...
export function FeedbackAdmin() {
  ...
  const stats = useMemo(() => {
    ...
    const average =
      records.length > 0 ? (records.reduce((sum, record) => sum + record.rating, 0) / records.length).toFixed(1) : "0.0"
        ...
        <aside className="border-b border-stone bg-ink p-4 text-white lg:border-b-0 lg:border-r lg:border-white/10">
          ...
          <nav className="mt-6 grid gap-2">
            {adminCopy.menu.map((item) => {
              const Icon = menuIcons[item.key as keyof typeof menuIcons]
              ...
              return (
                <button
                  className={
                    active
                      ...
                      : "flex h-11 items-center justify-between rounded-md px-3 text-sm font-semibold text-white/62"
                  }
                  disabled={!active}
                  key={item.key}
                  type="button"
                >
                  ...
                  {!active ? <span className="text-xs">{adminCopy.shell.reserved}</span> : null}
                </button>
              )
            })}
          </nav>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    {statusOrder.map((status) => (
                      <button
                        ...
                        type="button"
                      >
                        {updatingStatus === status ? <RefreshCw aria-hidden="true" className="h-4 w-4 animate-spin" /> : <SendHorizontal aria-hidden="true" className="h-4 w-4" />}
