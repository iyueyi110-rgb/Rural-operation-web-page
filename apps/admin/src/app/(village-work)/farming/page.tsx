...
export default function FarmingPage() {
  ...
  const [form, setForm] = useState({
    ...
    title: "荔枝春梢观察",
    description:
      "记录荔枝树春梢、花序和养护状态，为后续采摘和认养更新准备素材。",
  ...
  async function saveFarming() {
    setMessage("")
    try {
      await fetchAdminApi("/farming-calendar", {
      ...
      setMessage(adminCopy.farming.saved)
      await loadData()
      resetForm()
    } catch {
      setMessage(adminCopy.farming.saveFailed)
    }
  }
  ...
  const columns = useMemo<Array<TableColumn<FarmingRow>>>(
    () => [
      ...
      {
        key: "activityType",
        label: adminCopy.farming.activityType,
        render: (value) =>
          adminCopy.farming.activityTypes[
            value as keyof typeof adminCopy.farming.activityTypes
          ] ?? String(value ?? ""),
      },
      { key: "startDate", label: adminCopy.farming.startDate },
      {
        key: "status",
        label: adminCopy.farming.status,
        render: (value) =>
          adminCopy.farming.statuses[
            value as keyof typeof adminCopy.farming.statuses
          ] ?? String(value ?? ""),
      },
    ],
    [],
  ...
  return (
    <div className="grid gap-5">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-bold text-water">
            {adminCopy.farming.subtitle}
          </p>
          <h1 className="mt-1 text-2xl font-extrabold">
            {adminCopy.farming.title}
          </h1>
        </div>
      ...
      {isDemo ? (
        <AdminNotice>正式农事数据暂不可用，当前展示降级演示数据。</AdminNotice>
      ) : null}

      <div className="grid gap-3 md:grid-cols-3">
        <AdminStatCard
          icon={<Sunrise className="h-4 w-4" />}
          label={adminCopy.farming.title}
          value={isLoading ? "..." : rows.length}
        />
        <AdminStatCard
          label={adminCopy.farming.statuses.active}
          value={rows.filter((row) => row.status === "active").length}
        />
        <AdminStatCard
          label={adminCopy.farming.statuses.upcoming}
          value={rows.filter((row) => row.status === "upcoming").length}
        />
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
        <AdminDataTable
          columns={columns}
          emptyLabel={adminCopy.farming.noData}
          isLoading={isLoading}
        ...
        <section className="rounded-lg border border-stone bg-white p-5 shadow-soft">
          ...
          <div className="mt-4 grid gap-3">
            <select
              ...
              onChange={(event) =>
                setForm({ ...form, solarTerm: event.target.value })
              }
            ...
            <input
              ...
              placeholder={adminCopy.farming.activityTitle}
              value={form.title}
            />
              ...
              >
                {activityTypes.map((type) => (
                  <option key={type} value={type}>
                    {adminCopy.farming.activityTypes[type]}
                  </option>
                ))}
              </select>
              <select
              ...
              >
                {statuses.map((status) => (
                  <option key={status} value={status}>
                    {adminCopy.farming.statuses[status]}
                  </option>
                ))}
              </select>
            </div>
            ...
            <input
              className="h-10 rounded-md border border-stone bg-rice px-3"
              onChange={(event) =>
                setForm({ ...form, treeSpecies: event.target.value })
              }
              placeholder={adminCopy.farming.treeSpecies}
              value={form.treeSpecies}
            />
            <textarea
              className="min-h-28 rounded-md border border-stone bg-rice px-3 py-2"
              onChange={(event) =>
