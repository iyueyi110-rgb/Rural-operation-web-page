...
export default function InfrastructurePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const sensorPayload = (await sensorResponse.json()) as {
      data?: SensorReading[]
    }
  ...
  async function runDecision() {
    ...
    try {
      ...
      setMessage("设施调度建议已生成，请核对传感器读数后再决定是否采用。")
      await loadData()
    } catch {
      setMessage("设施调度建议没有生成，请检查传感器数据和网络后重试。")
    } finally {
      setIsDeciding(false)
    }
  }
  ...
  async function updateCommand(
    ...
    try {
      ...
      setMessage("建议状态已更新。标记执行前仍需确认现场已经完成操作。")
      await loadData()
    } catch {
      setMessage("建议状态没有更新成功，请检查网络后重试。")
    }
  }
  ...
  async function submitManualReading() {
    try {
      ...
      setMessage("手动读数没有保存，请检查传感器编号、数值和网络后重试。")
    }
  }
    ...
    <div className="grid gap-5">
      <header>
        ...
        <h1 className="mt-1 text-2xl font-extrabold">
          {adminCopy.infrastructure.title}
        </h1>
      </header>
      ...
      <section className="rounded-lg border border-stone bg-white p-5 shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-lg font-extrabold">
            {adminCopy.infrastructure.sensors}
          </h2>
          <button
            className="h-10 rounded-full border border-stone px-4 text-sm font-bold"
            onClick={loadData}
            type="button"
          >
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-5">
          {(sensors.length > 0 ? sensors : placeholderSensors()).map(
            (sensor) => {
              const meta = sensorMeta[
                sensor.type as keyof typeof sensorMeta
              ] ?? { label: sensor.type, icon: Gauge }
              const Icon = meta.icon
              const hasValue = sensors.length > 0
              return (
                <div
                  className="rounded-md bg-rice p-4"
                ...
                >
                  ...
                  <div className="mt-1 text-xs font-semibold text-ink/54">
                    {hasValue
                      ? new Date(sensor.createdAt).toLocaleString("zh-CN")
                      : adminCopy.infrastructure.pending}
                  </div>
                  <div
                    className={
                      hasValue
                        ? "mt-3 text-2xl font-extrabold"
                        : "mt-3 text-2xl font-extrabold text-ink/30"
                    }
                  >
        ...
        {sensors.length === 0 ? (
          <p className="mt-4 text-sm font-semibold leading-6 text-ink/56">
            {adminCopy.infrastructure.noSensorData}
          </p>
        ) : null}
      </section>
      ...
      <section className="rounded-lg border border-stone bg-white p-5 shadow-soft">
        ...
        >
          <input
            ...
            placeholder="传感器 ID"
            value={manualReading.sensorId}
          />
          <select
            className="h-10 rounded-md border border-stone bg-rice px-3"
            onChange={(event) =>
              setManualReading({ ...manualReading, type: event.target.value })
            }
          ...
          <input
            className="h-10 rounded-md border border-stone bg-rice px-3"
            onChange={(event) =>
              setManualReading({ ...manualReading, value: event.target.value })
            }
            placeholder="数值"
          ...
          <input
            className="h-10 rounded-md border border-stone bg-rice px-3"
            onChange={(event) =>
              setManualReading({ ...manualReading, unit: event.target.value })
            }
            placeholder="单位"
            value={manualReading.unit}
          />
          <button
      ...
      <section className="rounded-lg border border-stone bg-white p-5 shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-lg font-extrabold">
            {adminCopy.infrastructure.commands}
          </h2>
          <button
            className="h-10 rounded-full bg-ink px-4 text-sm font-bold text-white disabled:bg-ink/30"
            disabled={isDeciding}
            onClick={runDecision}
            type="button"
          >
            {isDeciding ? "正在生成建议" : "生成调度建议"}
          </button>
        </div>
        <div className="mt-4 grid gap-3">
          {commands.length > 0 ? (
            commands.map((command) => (
              <article
                className="rounded-md border border-stone bg-rice p-4"
                key={command.id}
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="text-xs font-bold text-water">
                    {command.status} · {command.triggeredBy}
                  </div>
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {command.status === "pending" ? (
                    <>
                      <button
                        className="rounded-full bg-moss px-3 py-1 text-xs font-bold text-white"
                      ...
                      <button
                        className="rounded-full border border-stone px-3 py-1 text-xs font-bold"
                        onClick={() => updateCommand(command.id, "rejected")}
                        {...adminWriteControlProps(canWrite)}
                        type="button"
                      >
                        不采用
                  ...
                  {command.status === "approved" ? (
                    <button
                      className="rounded-full bg-water px-3 py-1 text-xs font-bold text-white"
                      onClick={() => updateCommand(command.id, "executed")}
                      {...adminWriteControlProps(canWrite)}
                      type="button"
                    >
                      标记现场已执行
