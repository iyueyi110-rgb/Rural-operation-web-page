...
const shipmentStatuses: ShipmentRow["status"][] = [
  "pending",
  "picking",
  "shipping",
  "delivered",
...
export default function HarvestPage() {
  ...
  const loadRows = useCallback(async () => {
    setIsLoading(true)
    const payload = await fetchAdminApi<{ data?: HarvestRow[] }>(
      "/harvest-bookings",
    )
  ...
  useEffect(() => {
    ...
    setShipmentDraft({
      recipientName:
        selectedRow.shipment?.recipientName ?? selectedRow.guestName ?? "",
      recipientPhone:
        selectedRow.shipment?.recipientPhone ?? selectedRow.guestPhone ?? "",
      ...
      status: selectedRow.shipment
        ? getNextShipmentStatus(selectedRow.shipment.status)
        : "pending",
    })
  }, [selectedRow])
  ...
  async function updateStatus(id: string, status: string) {
    try {
      ...
      setMessage("采摘状态没有更新成功，请检查网络后重试。")
    }
  }
  ...
  async function updateDestination(row: HarvestRow, fruitDestination: string) {
    try {
      ...
      setMessage("果实去向没有更新成功，请检查网络后重试。")
    }
  }
  ...
  async function saveShipment() {
    ...
    try {
      await fetchAdminApi("/harvest-shipments", {
        ...
        body: JSON.stringify(
          selectedRow.shipment
            ? { id: selectedRow.shipment.id, ...payload }
            : payload,
        ),
      })
      ...
      setMessage("物流信息没有保存成功，请检查收件信息、快递单号和状态后重试。")
    }
  }
      ...
      render: (_value, row) => (
        <span className="flex flex-wrap gap-2">
          ...
          <button
            ...
            {...adminWriteControlProps(canWrite)}
            onClick={() => updateDestination(row, "加工")}
            type="button"
          >
            加工
          </button>
          <button
            ...
            {...adminWriteControlProps(canWrite)}
            onClick={() => updateDestination(row, "销售")}
            type="button"
          >
            销售
          </button>
          <button
            className="font-bold text-water"
  ...
  return (
    <div className="grid gap-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        <AdminDataTable
          columns={columns}
          emptyLabel={adminCopy.harvest.noData}
          isLoading={isLoading}
          rows={rows}
        />
        <aside className="rounded-lg border border-stone bg-white p-5 shadow-soft">
          ...
          <h2 className="mt-1 text-lg font-extrabold">
            {selectedRow
              ? `${selectedRow.treeId} / ${selectedRow.scheduledDate}`
              : "请选择采摘单"}
          </h2>
          <fieldset
          ...
          >
            <label className="grid gap-1 text-sm font-bold text-ink/70">
              收件人
              <input
                className="h-10 rounded-md border border-stone px-3"
                onChange={(event) =>
            ...
            <label className="grid gap-1 text-sm font-bold text-ink/70">
              ...
              <input
                ...
                value={shipmentDraft.recipientPhone}
              />
            </label>
            <label className="grid gap-1 text-sm font-bold text-ink/70">
              收货地址
              <textarea
                className="min-h-20 rounded-md border border-stone px-3 py-2"
                onChange={(event) =>
                  setShipmentDraft((draft) => ({
                    ...draft,
                ...
                value={shipmentDraft.recipientAddress}
              />
            </label>
            <label className="grid gap-1 text-sm font-bold text-ink/70">
              快递公司
            ...
            <label className="grid gap-1 text-sm font-bold text-ink/70">
              ...
              <select
                ...
                onChange={(event) =>
                  setShipmentDraft((draft) => ({
                    ...draft,
                    status: event.target.value as ShipmentRow["status"],
                  }))
                }
            ...
            <button
              className="mt-2 h-11 rounded-full bg-ink px-5 text-sm font-bold text-white disabled:opacity-50"
