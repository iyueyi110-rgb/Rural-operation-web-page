...
const villagersPageSource = readFileSync(
  new URL("../app/(village-work)/villagers/page.tsx", import.meta.url),
  "utf8",
)
...
test("handles unavailable villager APIs without an unhandled rejection", () => {
  ...
  assert.match(villagersPageSource, /fetchAdminApi<\{ data\?: VillagerRow\[\] \}>\("\/villagers"\)/)
  assert.match(villagersPageSource, /fetchAdminApi<\{ data\?: NodeRow\[\] \}>\("\/nodes"\)/)
