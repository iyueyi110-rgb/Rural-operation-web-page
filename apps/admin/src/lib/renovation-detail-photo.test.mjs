...
test("renovation strategy detail renders a schematic photo panel", () => {
  ...
  assert.match(pageSource, /getRenovationDemoPhoto/)
  assert.match(pageSource, /空间改造示意照片/)
  assert.match(pageSource, /role="img"/)
  assert.match(demoSource, /\/images\/renovation\/ai\/village-meal-granary\.jpg/)

  const photoUrls = Array.from(demoSource.matchAll(/url: "(\/images\/renovation\/[^"]+)"/g), (match) => match[1])
